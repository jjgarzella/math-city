...
#
# A thin host's whole contract is (a) ONE placeholder step, (b) the
# compose.expand that targets it with the canonical fragment, and (c) the var
# block that propagates the fragment's full var contract — mapping the host's
# `target` onto the fragment's `review_target` and every other var 1:1. This
# test pins all three, plus a NO-DRIFT cross-check: if the fragment gains or
# renames a var, the host must propagate it or this fails.

section "01 host integrity"

# --- top-level shape ---------------------------------------------------------
assert_eq "mol-standard-review" "$(formula_py formula-name)" "formula name is mol-standard-review"
...
assert_eq "expansion-standard-reviewer" "$(formula_py expand-with)" \
  "expand pulls in the canonical reviewer fragment"
...
while IFS= read -r key; do
  ...
  # value must be exactly {host_var}
  ref="${val#\{}"; ref="${ref%\}}"
  case "$val" in
    "{$ref}") : ;;
...
# Each host var's default equals the corresponding fragment var's default
# (target mirrors review_target).

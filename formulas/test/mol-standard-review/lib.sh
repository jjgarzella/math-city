# Shared helpers for the mol-standard-review deterministic test suite.
...
# (formula_py), a reader for the FRAGMENT's var contract (fragment_py — used for
# the no-drift cross-check), and gc-show helpers for the compile/expand tests.

# --- locations (run.sh exports these; provide safe fallbacks) ----------------
: "${SUITE_DIR:=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
: "${FORMULA:=$SUITE_DIR/../../mol-standard-review.toml}"
